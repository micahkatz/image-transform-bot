...
const controlBedLights = async (kind = 'SET_RED', args, res) => {
    ...
    return sendResponse(
        res,
        `❌ You can only change Micah\'s Bed Lights between 8:45 AM and 9:30 AM on a weekday 🥺🥺🥺`
    );
};
...
const controlDevice = async (
    ...
    if (kind.includes('SET_SCENE')) {
        ...
        const foundScene = lightScenes.find(
            (item) => item.value.id === numericalSceneId
        );
        return sendResponse(
            res,
            `✅ Set Micah\'s ${deviceName} to \`${
                foundScene?.name || 'Scene'
            }\``
        );
    }
    switch (kind) {
        ...
        case 'SET_BRIGHTNESS_HIGH':
            ...
            sendResponse(
                res,
                `✅ Set Brightness for Micah\'s ${deviceName} to 100\%`
            );
            break;
        case 'SET_BRIGHTNESS_MED':
            ...
            sendResponse(
                res,
                `✅ Set Brightness for Micah\'s ${deviceName} to 50\%`
            );
            break;
        case 'SET_BRIGHTNESS_LOW':
            ...
            sendResponse(
                res,
                `✅ Set Brightness for Micah\'s ${deviceName} to 10\%`
            );
            break;
        case 'ALERT':
            var shouldPowerOn = true;
            ...
            for (
                let intervalIndex = 0;
                intervalIndex < maxIterations;
                intervalIndex++
            ) {
                shouldPowerOn = !shouldPowerOn;
                const powerStateResponse = await changePowerState(
                    device,
                    shouldPowerOn
                );
                const textResponse = await powerStateResponse.text();
                console.log(
                    'powerstateresponse',
                    textResponse,
                    powerStateResponse.status,
                    powerStateResponse.ok
                );
                if (!powerStateResponse.ok) {
                    return sendResponse(
                        res,
                    ...
                    sendResponse(
                        res,
                        `✅ Alerting Micah by spamming his ${deviceName}`
                    );
                    isFirstTime = false;
                }
                await new Promise((r) => setTimeout(r, 2000));
